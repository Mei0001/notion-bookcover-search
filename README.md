...
2. **別の書籍を試す**
   - Google Books APIから提供される画像は書籍によって品質が異なります
   - 同じタイトルでも複数の版が検索結果に表示される場合があるので、別の版を試してみてください

3. **Notionの画像表示を確認**
   - Notionの「ファイルとメディア」プロパティにURLを貼り付け後、数秒待ってください
   - 画像が表示されない場合、URLを一度削除して再度貼り付けてみてください

4. **画像が読み込めない場合**
   - カード上の画像が📚アイコンで表示されている場合、そのURLは読み込みに失敗しています
   - 別の検索結果を試すか、検索キーワードを変更してみてください

### よくある質問

**Q: なぜAmazonの画像リンクは使えないのですか？**
A: AmazonのProduct Advertising APIは認証が必要で、利用規約上フロントエンドでの使用に制限があります。このツールでは認証不要のGoogle Books APIを使用しています。

**Q: 検索結果が少ない・見つからないのですが？**
A: 書籍名を変更して試してみてください。例えば、完全なタイトルではなく一部のキーワードで検索すると、より多くの結果が得られることがあります。
